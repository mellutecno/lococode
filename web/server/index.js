...
function buildInitialBackendPrompt(initialPrompt, projectMemory) {
  return [
    ...
    "",
    "JWT — REGOLA FERREA PER 'sub' CLAIM: python-jose (libreria che useremo per JWT) e' strict sui claim standard. Il claim 'sub' DEVE essere SEMPRE una STRINGA, mai un int. Pattern obbligatorio:",
    "  create_access_token(data={'sub': str(user.id)})   # ✓ giusto",
    "  create_access_token(data={'sub': user.id})        # ✗ ROMPE: jwt.decode lancia JWTError, login ok ma /me 401",
    "E nel get_current_user, riconvertire safely:",
    "  user_id_raw = payload.get('sub')",
    "  user_id = int(user_id_raw) if user_id_raw is not None else None",
    "Bug ricorrente: se sub e' int, /auth/login ritorna token valido MA tutte le rotte protette tornano 401 'Token non valido'. Il frontend cicla all'infinito su login -> dashboard -> auth/me 401 -> logout -> login.",
    "",
...
function buildInitialFrontendPrompt(initialPrompt, projectMemory) {
  return [
    ...
    "",
    "",
    "OVERRIDE FINALE DESIGN - QUESTE REGOLE VINCONO SU QUALSIASI INDICAZIONE VISIVA PRECEDENTE:",
    "- L'app generata NON deve sembrare figlia di LocoCode. Non copiare gradient, atmosfera, colori o layout di LocoCode.",
    "- Scegli una direzione visiva specifica per il dominio: sport performance, clinical clean, fintech sharp, hospitality warm, creative bold, SaaS operativo, consumer playful, ecc.",
    "- Varia palette e stile da progetto a progetto. Non usare sempre indigo/purple, non usare sempre dark glass, non usare sempre la stessa sidebar.",
    "- La coppia sfondo/testo deve essere deliberata: sfondo chiaro = testo scuro; sfondo scuro = testo chiaro. Niente testo chiaro su sfondo chiaro e niente testo scuro su sfondo scuro.",
    "- Se usi sfondi chiari, usa text-slate-900/800/700 e card bg-white con bordi/ombre. Se usi sfondi scuri, usa text-white/text-slate-100/200 e superfici scure ben definite.",
    "- Ogni view deve avere wrapper min-h-screen w-full con background coerente. Non lasciare buchi di body visibili nella preview.",
    "- Il file lc-theme.css e' solo un kit neutro di font/scrollbar/contrasto: NON usarlo come tema visivo da copiare.",
    "- Usa componenti UI preinstallati per velocita e coerenza, ma personalizza palette, spaziature, hero, sidebar e card con className coerenti con l'app.",
    "- Prima di consegnare, fai un controllo mentale di contrasto: ogni label, hint, badge, credenziale demo e testo secondario deve essere leggibile nello screenshot.",
    "",
    ...
    "",
    "VIETATO MODIFICARE body IN index.css con colori globali aggressivi. Il file lc-theme.css iniettato da LocoCode contiene solo font, scrollbar e guardrail di contrasto; il background vero deve stare nel wrapper React min-h-screen w-full della singola view.",
    "",
    "Aggiorna il piano dei task completati in questa fase.",
    "Restituisci solo blocchi file.",
  ].join("\n");
...
async function buildDesignReviewPrompt(target) {
  ...
  return [
    ...
    "",
    "## Identita visiva",
    "- NON usare indigo + purple come palette fissa. Scegli colori in base al dominio e rendi ogni app riconoscibile.",
    "- Se il frontend sembra un clone LocoCode, ridisegnalo: cambia palette, hero, sidebar, card, accenti e atmosfera.",
    "- Stati success: emerald o verde coerente, error: rose/red leggibile, warning: amber/orange leggibile, neutrals: slate/stone/zinc coerenti.",
    "- Controllo contrasto obbligatorio: ogni testo secondario, label, hint, badge e credenziale demo deve essere leggibile sul proprio sfondo.",
    "",
