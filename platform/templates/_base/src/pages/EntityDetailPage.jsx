...
import { PRIMARY_ENTITY, entityRoute, recordEditRoute, mc, statusTone } from "../lib/api.js";
...
export default function EntityDetailPage() {
  ...
  async function handleDelete() {
    ...
    const ok = window.confirm(`Eliminare definitivamente "${title || entity.label || "elemento"}"?`);
    if (!ok) return;
    setDeleting(true);
    setError(null);
    try {
      await mc.data(entity.name).delete(record.id);
      navigate(entityRoute(entity.name), { replace: true });
    } catch (err) {
  ...
  return (
    <div className="max-w-5xl mx-auto space-y-6">
      <Link to={entityRoute(entity?.name || entityName || PRIMARY_ENTITY)} className="inline-flex items-center gap-1.5 text-sm text-ink-400 hover:text-ink-100 transition">
      ...
      <section className="card relative overflow-hidden p-6 sm:p-8">
        ...
        <div className="relative flex flex-col sm:flex-row sm:items-center gap-5">
          <Avatar
            ...
            name={String(title || entity.label)}
            size="xl"
            ring
          ...
          <div className="flex items-center gap-2">
            <Link to={recordEditRoute(entity.name, record.id)} className="btn-secondary">
